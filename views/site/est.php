<html>
    <body>
		<div onclick="PartiCountdown()" style="position: absolute; bottom: 0; top:0; left: 0; right: 0;">
		...
		width: 100%;
		text-align: center;
		">Posizionare nell'altro schermo e fare clic per schermo intero e countdown</div>
    </body>
</html>
...
function PartiIlResto(){


	document.getElementById('canvas-dots').width = window.innerWidth;
	document.getElementById('canvas-dots').height = window.innerHeight;
...
var numberStage,
        ...
        countdownFrom = 5,
        testoFinale = '<?=  $winner?$winner:'¯\\\_(ツ)_/¯'?>',
		countdownTimer = 2000,
...
function drawNumber(num) {
  	 ...
  	 numberStageCtx.font = "bold "+window.innerWidth/6+"px Lato";
  	 numberStageCtx.fillText(num, numberStageWidth/2, 400);
...
function tweenDots(dot, pos, type) {
	...
	if (type === 'space') {
		...
		TweenMax.to(dot, (2.4 + Math.round(Math.random() * 100) / 100), {
		...
		// Tween dot to coordinate to form number
		TweenMax.to(dot, (1.2 + Math.round(Math.random() * 100) / 100), {
